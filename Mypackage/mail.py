...
def mail():
    ...
    def mailto(receivers):
        for people in receivers:
            ...
            file_names = []
            file_names.append(where_rootmenu + '/output/%(year)s/%(month)s月.txt' % {'year': nowyear, 'month': nowmonth})
            if int(nowday) < 7:
                try:
                    if nowmonth == 1:
                        if os.path.exists(where_script + '/output/%(year)s/12月.txt' %
                                {'year': str(int(nowyear) - 1)}):
                            file_names.append(where_rootmenu + '/output/%(year)s/12月.txt' % {'year': str(int(nowyear)-1)})
                    else:
                        if os.path.exists(where_script + '/output/%(year)s/%(month)s月.txt' %
                                {'year': nowyear,'month': str(int(nowmonth)-1)}):
                            file_names.append(where_rootmenu + '/output/%(year)s/%(month)s月.txt' %
                                          {'year': nowyear, 'month': str(int(nowmonth)-1)})
                except:
                    pass
